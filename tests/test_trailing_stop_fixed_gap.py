...
_trailing_stop_target returns (peak, candidate_stop) as of 2026-09-17
(explicit user instruction "Replace with LTP - Must have") - these tests
exercise the no-live-price (candle-close fallback) path, which is
unchanged in its own behavior; see test_trailing_stop_live_ltp.py for the
live-LTP-basis behavior itself."""
...
def test_returns_none_below_activation_threshold():
    ...
    _, cand = main._trailing_stop_target(today_df, 100.0, 98.0, _ENTRY_TS, _TZ_OFF)
    assert cand is None
...
def test_stop_stays_exactly_0_5R_below_the_peak_as_price_extends():
    ...
    peak, cand = main._trailing_stop_target(today_df, 100.0, 98.0, _ENTRY_TS, _TZ_OFF)
    assert cand == 105.0
    assert peak == 106.0
...
def test_stop_never_proposed_below_breakeven_floor():
    ...
    _, cand = main._trailing_stop_target(today_df, 100.0, 99.2, _ENTRY_TS, _TZ_OFF)
    # activation not yet reached at 100.2 (needs 100.4)
    assert cand is None
    today_df2 = _today_df([100.0, 100.4])
    _, cand2 = main._trailing_stop_target(today_df2, 100.0, 99.2, _ENTRY_TS, _TZ_OFF)
...
def test_r_le_0_returns_none_never_raises():
    ...
    assert main._trailing_stop_target(today_df, 100.0, 100.0, _ENTRY_TS, _TZ_OFF) == (None, None)
    assert main._trailing_stop_target(today_df, 100.0, 101.0, _ENTRY_TS, _TZ_OFF) == (None, None)
