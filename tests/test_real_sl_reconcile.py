...
def test_dead_sl_is_detected_and_a_fresh_one_placed_in_the_same_tick():
    # The core fix: a resting SL found dead on reconcile must not just
    # start the degraded clock and wait - it must feed the SAME tick's
    # existing retry path immediately (sl_trigger_price cleared too, not
    # just sl_order_id, so the "no upward move" gate doesn't skip it).
    _fresh_db()
